...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'littlelambocoin-dashboard-satellite';
    ...
    this.connection = new Connection(daemonAddress, {
      ...
      timeoutInSeconds: 20,
      coin: 'littlelambocoin'
    });

    this.connection.addService(constants.SERVICE('littlelambocoin').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('littlelambocoin')[service]);
